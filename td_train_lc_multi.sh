      ...
      do
		...
		if [ $proc_frames -eq $extra_frames1 ];then
			...
			if [ $extra_frames1 -eq $extra_frames2 ]; then
			    ...
			    snapdir=./td/lc_model_d${lstm_depth}_d${units}_f${filters}_l${learn_rate}_B${batch_size}_D${dropout}_f${factor}_LNtrue_vgg_lstm_
			    snapdir=${snapdir}p${proc_frames}_e1${extra_frames1}_e2${extra_frames2}_${optim}_${direction}
			    logdir=./td/lc_logs_d${lstm_depth}_d${units}_f${filters}_l${learn_rate}_B${batch_size}_D${dropout}_f${factor}_LNtrue_vgg_lstm_
