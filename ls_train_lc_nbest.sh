        ...
        do
          ...
          do
		      ...
		      if [ $extra_frames1 -eq $extra_frames2 ];then
			         ...
			         snapdir=${snapdir}_p${proc_frames}_ef${extra_frames1}_es${extra_frames2}_n${num_extra_frames1}_${optim}_nb${nbest}_${direction}
			         logdir=./ls/lc_logs_d${lstm_depth}_d${units}_f${filters}_l${learn_rate}_B${batch_size}_D${dropout}_f${factor}_LNtrue_vgg_lstm
			         logdir=${logdir}_p${proc_frames}_ef${extra_frames1}_es${extra_frames2}_n${num_extra_frames1}_${optim}_nb${nbest}_${direction}
			         mkdir -p $snapdir
			         mkdir -p $logdir

			         python nbest_multi_lc_lstm.py --data $train --valid $valid \
