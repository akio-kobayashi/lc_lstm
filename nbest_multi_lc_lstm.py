...
import nbest_multi_fixed_generator
import vgg2l
import vgg1l
import multi_utils
...
def main():
    ...
    training_generator = nbest_multi_fixed_generator.NbestFixedDataGenerator(
        args.data, args.key_file, args.batch_size, args.feat_dim, args.n_labels,
        args.process_frames, args.extra_frames1, args.extra_frames2, args.num_extra_frames1, args.nbest)
    valid_generator =nbest_multi_fixed_generator.NbestFixedDataGenerator(
    ...
    with open(args.log_dir+'/logs', 'w') as logs:
        for ep in range(args.epochs):
            ...
            for bt in range(training_generator.__len__()):
                ...
                x, mask, numer_labels, denom_labels = training_generator.__getitem__(bt)
                model.reset_states()
                for b in range(x.shape[0]):
                    ...
                    loss,acc = model.train_on_batch(x=[x_in,mask_in], y=[numer_labels, denom_labels])
                    # for micro-mean
            ...
            for bt in range(valid_generator.__len__()):
                x,mask,numer_labels,denom_labels = valid_generator.__getitem__(bt)
