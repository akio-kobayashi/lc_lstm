...
from keras.layers import TimeDistributed, Bidirectional, Dropout, Lambda, Masking, Concatenate, Add
import keras.utils
...
def build_model(inputs, mask, units, depth, n_labels, feat_dim, init_lr,
    ...
    outputs1, outputs2, outputs3, mask2, mask3 = vgg_lstm.VGG3(masked, mask, init_filters, feat_dim)

    outputs1 = Lambda(lambda x: tf.multiply(x[0], x[1]))([outputs1, mask])
    outputs1 = Masking(mask_value=0.0)(outputs1)

    outputs2 = Lambda(lambda x: tf.multiply(x[0], x[1]))([outputs2, mask2])
    outputs2 = Masking(mask_value=0.0)(outputs2)
...
def main():
    ...
    mask_dim=args.feat_dim * args.filters * 2
    inputs = Input(batch_shape=(args.batch_size, None, args.feat_dim))
    masks = Input(batch_shape=(args.batch_size, None, mask_dim))
