...
if [ $host == "brandy" ];then
    ...
    weights=ls/ce_model_d4_d256_f32_l1.0_LNtrue_BNtrue_B16_D0.0_f0.9_vgg_lstm_adadelta_bi/snapshot.h5
else
      ...
      do
        ...
        do
		  ...
		  do
			  ...
			  snapdir=./ls/lc_part_model_d${lstm_depth}_d${units}_f${filters}_l${learn_rate}_B${batch_size}_D${dropout}_f${factor}_LNtrue_vgg_lstm
			  snapdir=${snapdir}_p${proc_frames}_ef${extra_frames1}_es${extra_frames2}_n${num_extra_frames1}_${optim}_${direction}
			  logdir=./ls/lc_part_logs_d${lstm_depth}_d${units}_f${filters}_l${learn_rate}_B${batch_size}_D${dropout}_f${factor}_LNtrue_vgg_lstm
			  logdir=${logdir}_p${proc_frames}_ef${extra_frames1}_es${extra_frames2}_n${num_extra_frames1}_${optim}_${direction}
			  mkdir -p $snapdir
			  mkdir -p $logdir

			  python part_lc_lstm.py --data $train --valid $valid \
